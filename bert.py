...
class BertLayer(nn.Module):
  ...
  def add_norm(self, input, output, dense_layer, dropout, ln_layer):
    ...
    ### TODO
    x = dropout(dense_layer(output)) + input
    x = ln_layer(x)
    return x
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    x = self.add_norm(hidden_states, multi_head_attention, self.attention_dense, self.attention_dropout, self.attention_layer_norm)
    y = self.interm_af(self.interm_dense(x))
    z = self.add_norm(x, y, self.out_dense, self.out_dropout, self.out_layer_norm)
    return z
...
class BertModel(BertPreTrainedModel):
  ...
  def embed(self, input_ids):
    ...
    ### TODO
    x = input_embeds + pos_embeds + tk_type_embeds
